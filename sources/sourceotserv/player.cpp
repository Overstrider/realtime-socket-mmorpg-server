...
void Player::onDie()
{
	if(getZone() != ZONE_PVP){
		...
		DeathList killers = getKillers(g_config.getNumber(ConfigManager::PVP_ASSIST_COUNT));
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	if(g_config.getNumber(ConfigManager::KILLS_TO_BAN) != 0 && skullTicks > (g_config.getNumber(ConfigManager::KILLS_TO_BAN) - 1) * g_config.getNumber(ConfigManager::FRAG_TIME))
	{
		if (g_bans.addPlayerBan(getName(), std::time(NULL) + g_config.getNumber(ConfigManager::BAN_LENGTH), 1 /* Type here your gamemaster ID */, "Automatic", "", 28, ACTION_BANISHMENT))
		{
			...
			g_scheduler.addEvent(createSchedulerTask(500, boost::bind(&Player::kickPlayer, this)));
		}
	}
	else if(getSkull() != SKULL_RED)
	{
		if(getSkull() != SKULL_RED && g_config.getNumber(ConfigManager::KILLS_PER_DAY_RED_SKULL) != 0 && skullTicks > (g_config.getNumber(ConfigManager::KILLS_PER_DAY_RED_SKULL) - 1) * g_config.getNumber(ConfigManager::FRAG_TIME))
		{
			setSkull(SKULL_RED);
			g_game.updateCreatureSkull(this);
		}
...
void Player::checkSkullTicks(int32_t ticks)
{
	...
	if (skullTicks < 1000 && !hasCondition(CONDITION_INFIGHT) && skull != SKULL_NONE) {
		setSkull(SKULL_NONE);
		g_game.updateCreatureSkull(this);
	}
}
