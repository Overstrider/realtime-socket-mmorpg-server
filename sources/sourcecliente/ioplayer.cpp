...
bool IOPlayer::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	query << "SELECT `players`.`id` AS `id`, `players`.`name` AS `name`, \
		...
		`posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, \
		`skull`, `loss_experience`, `loss_mana`, `loss_skills`, ";
	...
	int32_t redSkullSeconds = result->getDataInt("skulltime") - std::time(NULL);
	if (redSkullSeconds > 0) {
		// ensure that we round up the number of ticks
		player->skullTicks = (redSkullSeconds + 2) * 1000;

		if (result->getDataInt("skull") == 1) {
...
bool IOPlayer::savePlayer(Player* player, bool shallow)
{
	...
	if (player->skullTicks > 0) {
		skullTime = std::time(NULL) + player->skullTicks / 1000;
	}

	query << ", `skulltime` = " << skullTime;
	query << ", `skull` = " << (player->skull == SKULL_RED ? 1 : 0);
#endif
