function onLogin(cid)
	...
	local events = {
		...
		"Look", -- Register on look player
		"Killed", --Register Task System
		"HPadvance" --Register HP Advanced
	}
	...
	if getPlayerStorageValue(cid, getConfigInfo("storage_sendrook")) < 1 then
		...
		doAddContainerItem(bp, 2398, 1) -- sabre
		doAddContainerItem(bp, 2512, 1) -- shield
	...
	setPlayerStorageValue(cid, 984274497, 1)	
	end

	-- Player is not premium - remove premium privileges
	if(getPlayerPremiumDays(cid) <= 0 and getPlayerStorageValue(cid, 984274497) == 1) then
		...
		setPlayerStorageValue(cid, 984274497, 0)
	end
